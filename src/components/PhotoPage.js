 ...
 filtersApplied: false,
  ...
  _filterPhotoData = () => {
    ...
    for (let i = 0; i < data.length; i++) {
      const photoSet = data[i];
      for (let j = 0; j < photoSet.photos.length; j++) {
        let photo = photoSet.photos[j];
        let isValid = true;
        for (let g in this.state.filters.genres) {
